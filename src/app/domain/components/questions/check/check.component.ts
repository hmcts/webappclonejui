...
export class CheckQuestionsComponent implements OnInit {
    ...
    onSubmit() {
        this.questionService.sendQuestions(this.caseId, this.roundNumber).subscribe(res => {
            this.redirectionService.redirect(`/jurisdiction/${this.jurisdiction}/casetype/${this.caseType}/${this.caseId}/questions?sent=success`);
        }, () => {
            this.redirectionService.redirect(`/jurisdiction/${this.jurisdiction}/casetype/${this.caseType}/${this.caseId}/questions?sent=failure`);
        });
    }
}
